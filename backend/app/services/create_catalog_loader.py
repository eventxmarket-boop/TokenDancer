...
def _normalize_create_catalog_group(group: dict[str, Any]) -> dict[str, Any]:
    if group.get("group") != "relationship_intimate":
        return group

    ordered_items = sorted(
        (_normalize_intimate_item(item) for item in group.get("items", []) if isinstance(item, dict)),
        key=lambda item: int(item.get("sort_order") or 0),
    )
    deduped: dict[str, dict[str, Any]] = {}
    for item in ordered_items:
        slug = _normalize_intimate_slug(str(item.get("slug") or "").strip())
        existing = deduped.get(slug)
        if existing is None:
            deduped[slug] = {
                **item,
                "slug": slug,
                "name": "关系经营" if slug == INTIMATE_CANONICAL_SLUG else item.get("name", ""),
            }
            continue
        ...
        deduped[slug] = {
            ...
            **item,
            "slug": slug,
            "name": "关系经营" if slug == INTIMATE_CANONICAL_SLUG else existing.get("name", ""),
