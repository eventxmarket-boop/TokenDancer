...
const TARGET_NAME = process.env.MESC_BLOG_TARGET_NAME ?? 'Mescladís Blog';
const TARGET_URL = normalizeTargetUrl(process.env.MESC_BLOG_TARGET_URL ?? 'https://mescladis.org/blog/');
const POLL_INTERVAL_MS = readInteger('MESC_BLOG_POLL_INTERVAL_MS', 60_000);
const LANE_INTERVAL_MS = readInteger('MESC_BLOG_LANE_INTERVAL_MS', POLL_INTERVAL_MS);
const LANE_START_OFFSETS_MS = parseLaneOffsets(process.env.MESC_BLOG_LANE_START_OFFSETS_MS ?? '0');
const PAGE_TIMEOUT_MS = readInteger('MESC_BLOG_PAGE_TIMEOUT_MS', 30_000);
const PAGE_STABILIZE_MS = readInteger('MESC_BLOG_PAGE_STABILIZE_MS', 2_000);
const HEADLESS = (process.env.MESC_BLOG_HEADLESS ?? 'true').toLowerCase() !== 'false';
const ALERT_FILE_PATH = (process.env.MESC_BLOG_ALERT_FILE_PATH ?? '').trim();
const TELEGRAM_BOT_TOKEN = (process.env.TELEGRAM_BOT_TOKEN ?? '').trim();
const TELEGRAM_CHAT_ID = (process.env.TELEGRAM_CHAT_ID ?? '').trim();
const STATE_FILE = path.join(__dirname, process.env.MESC_BLOG_STATE_FILE ?? 'mescladis_blog_state.json');
  ...
  try {
    ...
    await runSerializedLaneScheduler({
      ...
      onLaneStart: ({ laneIndex, offsetMs }) => {
        console.log(
          `[MESC_BLOG] lane start | lane=${laneIndex} | offsetMs=${offsetMs} | target=${TARGET_NAME}`,
        );
      },
      onTick: async ({ laneIndex, cycle }) => {
        const roundStartedAt = new Date().toISOString();
        console.log(
          `[MESC_BLOG] lane #${laneIndex} cycle #${cycle} start | target=${TARGET_NAME} | at=${roundStartedAt}`,
        );

        const snapshot = await probeBlogPage();
        ...
        if (!previousHash) {
          ...
          console.log(`[MESC_BLOG] baseline seeded | hash=${currentHash}`);
          ...
          saveState(STATE_FILE, state);
          console.log(`[MESC_BLOG] unchanged | hash=${currentHash}`);
        }
      },
    });
  } finally {
...
async function probeBlogPage() {
  ...
  try {
    ...
    console.log(`[MESC_BLOG][ERROR] ${message}`);
    return {
...
async function pushToTelegram(event) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.log(
        `[MESC_BLOG][WARN] telegram push failed | status=${response.status} | ${detail.trim() || 'no response body'}`,
      );
    }
  } catch (error) {
    console.log(
      `[MESC_BLOG][WARN] telegram push error | ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
