<template>
  <div class="page-container">
    ...
    <div class="page-hint">以下内容为前台 /privacy 页面的生效版本</div>

    <div v-if="loading" class="state-msg">加载中…</div>
    <div v-else-if="error" class="state-msg error">{{ error }}</div>
    <div v-else>
      <div class="form-group"><label>标题</label><input class="form-input" v-model="form.title" placeholder="隐私政策" /></div>
      <div class="form-group"><label>正文</label><textarea class="form-textarea" v-model="form.content" placeholder="请输入隐私政策内容…" rows="20"></textarea></div>
...
const fetch = async () => {
  ...
  try {
    page.value = await adminContentApi.getPrivacy()
...
const doSave = async () => {
  saving.value = true
  try {
    await adminContentApi.updatePrivacy(form)
    feedback.success('隐私政策已保存')
    fetch()
