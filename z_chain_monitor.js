...
if (!process.env.MONTJUIC_STATE_FILE) {
  process.env.MONTJUIC_STATE_FILE = `${chainBaseDir}/${chainInstanceId}_state.json`;
}
if (!process.env.MONTJUIC_HEARTBEAT_FILE_PATH) {
  process.env.MONTJUIC_HEARTBEAT_FILE_PATH = `${chainBaseDir}/${chainInstanceId}_heartbeat.json`;
}

if (!process.env.MONTJUIC_TARGET_URL?.trim()) {
  console.error('[Z_CHAIN] MONTJUIC_TARGET_URL is required (set Z_CHAIN_TARGET_URL first)');
  await new Promise(() => {});
}
...
if (!process.env.MONTJUIC_AUTOFILL_TRIGGER_MODE) {
  process.env.MONTJUIC_AUTOFILL_TRIGGER_MODE = 'any_open_snapshot';
}
...
if (!process.env.MONTJUIC_TELEGRAM_DISABLED) {
  process.env.MONTJUIC_TELEGRAM_DISABLED = 'true';
}

if (!process.env.MONTJUIC_SUPPRESS_STATUS_EVENTS) {
  process.env.MONTJUIC_SUPPRESS_STATUS_EVENTS = 'false';
}
